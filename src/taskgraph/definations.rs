use crate::{BufferID, CommandRecorder, ComputePipeline, Device, ImageAspect, ImageID, ImageLayout, ImageViewID, RasterizationPipeline, Swapchain};
...
impl Task {
    pub(crate) fn contains_image(&self, i: usize) -> bool {
        for res in &self.resources {
            match res {
                TaskResource::Image(img) => {
                    if img.id.0 == i {
                        return true;
                    }
                }
